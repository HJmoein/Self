...
async def show_user_id(event):
    ...
    if reply is None:
        await event.edit("برای گرفتن آیدی، روی پیام شخص ریپلای کن و بنویس: .آیدی")
        return
...
async def download_story_link(chat_id, link, status_message=None):
    ...
    try:
        ...
        result = await client(
            GetStoriesByIDRequest(peer=peer, id=[story_id])
        )
        ...
        def show_progress(downloaded, total):
            ...
            now = time.monotonic()
            if status_message is None or now - last_update < 1 and downloaded < total:
                return
            last_update = now
            ...
            frame_index += 1
            asyncio.create_task(status_message.edit(message))

        with tempfile.TemporaryDirectory(prefix="meow_story_") as folder:
            ...
            await client.send_file(chat_id, file_path, caption="استوری دانلود شد ✅")
        return None
...
async def download_channel_message(chat_id, link, status_message=None):
    ...
    try:
        ...
        author_name = getattr(message, "post_author", None)
        if not author_name:
            try:
        ...
        if message.media:
            ...
            def show_progress(downloaded, total):
                ...
                frame_index += 1
                asyncio.create_task(status_message.edit(progress))

            with tempfile.TemporaryDirectory(prefix="channel_download_") as folder:
            ...
            await run_with_floodwait(
                lambda: client.send_message(chat_id, message_info + message.message)
            )
        else:
...
async def handler(event):
    ...
    if text.startswith("دانلود چنل "):
        ...
        await event.edit("⏳ دانلود پست کانال شروع شد...\n[░░░░░░░░░░] 0%")
        error_message = await download_channel_message(chat_id, link, event)
        if error_message:
    ...
    if text.startswith(STORY_COMMAND + " "):
        ...
        if not STORY_LINK_PATTERN.match(link):
            ...
            return
        await event.edit("⏳ دانلود استوری شروع شد...\n[░░░░░░░░░░] 0%")
        error_message = await download_story_link(chat_id, link, event)
        if error_message:
            await event.edit(error_message)
        else:
            await event.delete()
    ...
    if text == "میو روشن":
        ...
        if reply is None or not reply.media:
            await event.edit(
                "روی استوری یا پیام استوری ریپلای کن و دوباره بنویس: .استوری دانلود"
            )
            return

        await event.edit("در حال دانلود استوری... ⏳")
        file_path = None
        try:
            with tempfile.TemporaryDirectory(prefix="meow_story_") as folder:
                file_path = await client.download_media(reply, file=folder)
                if not file_path:
                    await event.edit("دانلود این استوری ممکن نیست.")
                    return
                await client.send_file(chat_id, file_path, caption="استوری دانلود شد ✅")
            await event.delete()
        except Exception as error:
            await event.edit(f"دانلود استوری انجام نشد: {type(error).__name__}")
